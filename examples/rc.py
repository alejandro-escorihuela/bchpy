...
if __name__ == "__main__":
    ...
    print("RKN: Ordre 6 projectat, ordre 6 sense projectar. nac10")
    nom = "nac10"
    t0 = tm.time()
    met = Metode(6)
    cc = [a[0], b[0].expand(complex=True), \
          a[1], b[1].expand(complex=True), \
          a[2], b[2].expand(complex=True), \
          a[3], b[3].expand(complex=True), \
          a[4], b[4].expand(complex=True), \
          a[5], \
          b[4].conjugate().expand(complex=True), a[4], \
          b[3].conjugate().expand(complex=True), a[3], \
          b[2].conjugate().expand(complex=True), a[2], \
          b[1].conjugate().expand(complex=True), a[1], \
          b[0].conjugate().expand(complex=True), a[0]]
    met.setABA(*cc, debug = True)
    met.collectI()
    # met.exportpy(nom + ".py")
    t1 = tm.time()
    print("\tComposició ABA: 21 fluxes ->", nom, "\t(" + str(t1-t0) + " s)") 
