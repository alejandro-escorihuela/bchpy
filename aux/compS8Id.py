...
# alex
# compS8Id.py
# Identitats (relacions de commutació) dels corxets de la base per a composicions de S8
...
if __name__ == "__main__":
    ...
    Z = llegir_baseS2("compS8_base.txt", S)
        ...
        for ind in (llistes):
            ...
            for j, l in itertools.product(range(1, len(Z[i])), range(1, len(Z[k]))):
                if not (i == k and j > l):
                    ...
                    if sol:
                        cad = "[M8" + str(i) + str(j) + ", M8" + str(k) + str(l) + "] = "
                        cadrel = "relM8[" + str(i) + "][" + str(j) + "][" + str(k) + "][" + str(l) + "] = ["
                        # cadtex = "\left[M8_{" + str(i) + "," + str(j) + "},M8_{" + str(k) + "," + str(l) + "} \\right] = "
                        for m in sol:
                            if sol[m] != 0:
                                if sol[m] < 0:
                                    cad += str(sol[m]) + "*M8" + str(n) + str(c[n].index(m) + 1)
                                else:
                                    cad += "+" + str(sol[m]) + "*M8" + str(n) + str(c[n].index(m) + 1)
                                ...
                                # cadtex += "+" + factex + "M8_{" + str(n) + "," + str(c[n].index(m) + 1) + "}"
                        cadrel = cadrel[:-2] + "]"
                        print(cad)
                        if impfits:
                            fitR.write(cadrel + "\n")
