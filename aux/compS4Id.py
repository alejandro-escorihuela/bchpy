...
# alex
# compS4Id.py
# Identitats (relacions de commutació) dels corxets de la base per a composicions de S4
...
if __name__ == "__main__":
    ...
    Z = llegir_baseS2("compS4_base.txt", S)
        ...
        for ind in (llistes):
            ...
            for j, l in itertools.product(range(1, len(Z[i])), range(1, len(Z[k]))):
                if not (i == k and j > l):
                    ...
                    if sol:
                        cad = "[M4" + str(i) + str(j) + ", M4" + str(k) + str(l) + "] = "
                        cadrel = "relM4[" + str(i) + "][" + str(j) + "][" + str(k) + "][" + str(l) + "] = ["
                        # cadtex = "\left[M4_{" + str(i) + "," + str(j) + "},M4_{" + str(k) + "," + str(l) + "} \\right] = "
                        for m in sol:
                            if sol[m] != 0:
                                if sol[m] < 0:
                                    cad += str(sol[m]) + "*M4" + str(n) + str(c[n].index(m) + 1)
                                else:
                                    cad += "+" + str(sol[m]) + "*M4" + str(n) + str(c[n].index(m) + 1)
                                ...
                                # cadtex += "+" + factex + "M4_{" + str(n) + "," + str(c[n].index(m) + 1) + "}"
                        cadrel = cadrel[:-2] + "]"
                        print(cad)
                        if impfits:
                            fitR.write(cadrel + "\n")
